const translations = {
  en: {
    ...
    "demo.desc":
      "Builds a complete interaction path from <strong>cover animation, global statistics, gallery filtering</strong> to <strong>individual image detail analysis</strong>, providing analytical views including <strong>count statistics, co-occurrence matrix, network graph, and color clustering</strong>; supports filtering by dynasty, location, and other dimensions, and allows users to inspect <strong>metadata, representative patterns, local regions, color analysis, cave location</strong>, and <strong>related samples</strong> for each image.",
    "role.kicker": "Work",
    "role.title": "Contribution",
    "role.frontend.title": "Frontend system",
    "role.frontend.desc":
      'As the primary implementer, I completed the frontend page design and implementation, constructing a complete interaction flow from <strong>homepage narrative, global statistics, gallery filtering to individual image detail analysis</strong>.',
    ...
    "role.data.desc":
      'Oriented around frontend interaction and visualization needs, I organized image metadata, pattern labels, and statistical results, transforming raw research data into <strong>structured data that directly drives filtering, charts, and detail page rendering</strong>.',
    ...
    "role.visual.desc":
      'Responsible for the overall <strong>UI and information architecture design</strong>, translating high-density cultural heritage image data into a browsable, filterable, and analyzable interface experience while balancing academic expression and web usability.',
  ...
  zh: {
    ...
    "demo.title": "系统概览",
    "demo.desc":
      "搭建从<strong>封面动画、全局统计、图库筛选</strong>到<strong>单图详情分析</strong>的完整交互路径，提供<strong>计数统计、共现矩阵、网络图、颜色聚类</strong>等多种分析视图；支持按朝代、位置等维度筛选，可查看单张图像的<strong>元信息、典型纹样、局部区域、色彩分析、所在洞窟</strong>以及<strong>相关样本</strong>。",
    "role.kicker": "工作",
    "role.title": "贡献",
    "role.frontend.title": "前端系统",
    "role.frontend.desc":
      '作为主要实现者，我完成了项目前端页面设计与实现，构建了从<strong>首页叙事、全局统计、图库筛选到单图详情分析</strong>的完整交互流程。',
    ...
    "role.data.desc":
      '围绕前端交互与可视化需求，整理图像元信息、纹样标签与统计结果，将原始研究数据转换为<strong>可直接驱动筛选、图表和详情页渲染的结构化数据</strong>。',
    "role.data.li1": "分类纹样、图像元信息和整理统计字段。",
    "role.data.li2": "实现纹样计数、共现关系、颜色聚类等分析结果。",
    "role.data.li3": "分析输出并映射到所需的数据格式。",
    "role.visual.title": "可视化设计",
    "role.visual.desc":
      '负责整体 <strong>UI 与信息架构设计</strong>，将高密度的文化遗产图像数据转译为可浏览、可筛选、可分析的界面体验，在学术信息表达与网页可用性之间取得平衡。',
